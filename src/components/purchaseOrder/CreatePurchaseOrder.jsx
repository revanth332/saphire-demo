      ...
      <div style={styles.contentArea}>
        ...
        <div style={styles.previewPane}>
          <div style={styles.previewHeader}>
            ...
            ><Send size={18} style={{ marginRight: "5px" }} /> Send PO</button>
          </div>
...
// const POGenerationScreen = ({
//   vendorName = "Default Vendor Inc.",
//   vendorEmail = "sales@defaultvendor.com",
// }) => {
//   const [messages, setMessages] = useState([]);
//   const [inputValue, setInputValue] = useState("");
//   const [poPreviewContent, setPoPreviewContent] = useState(null);
...
//     isBotSpeakingRef.current = isBotSpeaking;
//   }, [isBotSpeaking]);
//   const isBotTypingRef = useRef(isBotTyping);
//   useEffect(() => {
//     isBotTypingRef.current = isBotTyping;
//   }, [isBotTyping]);
...
//       setPoPreviewContent("1");
//       setIsRfpUploading(false);
//       setRfpFiles(prev => [...prev, { name: file.name, content: "1", summary: <SummaryList id={0} /> }]); // Added a dummy summary
//       setConversationStage(1);
//       setIsConfirmationVisible(true)
//     }, 1000)
//   }
...
//     w-[60px] h-[60px] rounded-full border flex items-center justify-center shadow-md transition-colors duration-200 ease-in-out
//     ${micDisabled ? 'cursor-not-allowed bg-gray-200 text-gray-400 border-gray-300' : 'cursor-pointer'}
//     ${isBotSpeaking || isBotTyping ? 'bg-gray-200 text-gray-400 border-gray-300' : ''}
//     ${isListening ? 'bg-red-500 text-white border-red-500' : ''}
//     ${!isListening && userWantsToListen && speechSupported ? 'bg-gray-600 text-white border-gray-600' : ''}
//     ${!isListening && !userWantsToListen && speechSupported ? 'bg-gray-50 text-gray-600 border-gray-300' : ''}
//   `;
...
//   return (
//     <div className="flex flex-col font-sans h-screen mx-auto border border-gray-300 shadow-lg overflow-hidden">
//       <div className="py-3 px-5 border-b border-gray-200 bg-gray-50">
//         <div className="text-2xl font-bold mb-1">Generate PO</div>
//         {/* <div className="text-sm text-gray-600 mb-2">Use voice to interact with the chatbot for Purchase Orders.</div> */}
//       </div>

//       <div className="flex flex-1 overflow-hidden w-full flex-col md:flex-row">

//         <div className="flex flex-col bg-white text-gray-800 relative md:w-1/2 border-r border-gray-200">
//           <div className="p-5">
...
//             {isConfirmationVisible && <>
//               <div className="bg-white/70 absolute inset-0 z-10"></div>
//               <div className="absolute inset-0 z-20 flex justify-center items-center flex-col gap-3">
//                 <p>Get Started with Voice Assisted Journey?</p>
//                 <Button className="text-miracle-darkBlue border border-miracle-darkBlue hover:text-miracle-darkBlue" onClick={handleContinuePoGeneration} variant={"outline"}>Continue</Button>
//               </div>
//             </>}

//             <div className={`${selectedRfpFileIndex !== undefined ? "h-[220px]" : "h-[345px]"} flex-1 overflow-y-auto flex flex-col gap-2.5 mb-4 pr-1`}>
//               {messages.map((msg) => (
//                 <div key={msg.id} className={`p-2.5 px-3.5 rounded-2xl max-w-[80%] break-words text-sm leading-normal shadow-sm
//                   ${msg.sender === "user" ? 'bg-blue-500 text-white ml-auto rounded-br-sm' : 'bg-gray-200 text-gray-800 mr-auto rounded-bl-sm'}`}>
//                   {msg.text}
//                 </div>
//               ))}
//               <div ref={chatMessagesEndRef} />
//             </div>
//             {!isConfirmationVisible && <div className="text-xl font-bold text-center text-gray-700 min-h-[30px] leading-tight relative overflow-hidden">
//               {animatingOutPrompt.text && (<div key={animatingOutPrompt.key} className="absolute inset-0 text-inherit font-inherit leading-inherit p-inherit animate-moveUpAndFadeOut">{animatingOutPrompt.text}</div>)}
//               <div key={`current-${currentDisplayPromptText}-${messages.filter((m) => m.sender === "bot").length}`} className="text-inherit font-inherit leading-inherit p-inherit animate-fadeInCurrent opacity-0">{currentDisplayPromptText}</div>
//             </div>}
//             <div className="text-base font-medium text-center py-4 min-h-[30px] flex flex-col items-center justify-center text-gray-800 leading-tight w-full">
//               {speechSupported ? inputValue || "\u00A0" : "Speech input unavailable."}
//             </div>
//             <div className="flex flex-col items-center pb-0 mt-auto">
//               <div ref={siriContainerRef} className={`w-[80%] h-[50px] flex items-center justify-center relative ${!isBotSpeaking && !isListening ? "invisible" : ""}`}></div>
//               <div className="text-red-600 text-sm text-center mb-2 min-h-[18px] font-bold">{speechError}</div>
//               <div className="w-full flex flex-col">
//                 <div className="w-full flex justify-center gap-5">
//                   <div className="flex flex-col items-end">
//                     <button
...
//                   </div>
//                 </div>
//                 <div className="text-gray-600 text-sm text-center mt-3 min-h-[20px]">{!speechError ? micStatusText : ""}</div>
//               </div>
//             </div>
//           </div>
//         </div>

//         <div className="flex flex-col p-0 bg-white flex-1 md:w-1/2">
//           <div className="flex justify-between items-center p-5 border-b border-gray-200">
//             <div className="text-lg font-bold">PO Preview</div>
//             <button
//               className={`py-2.5 px-4 bg-green-600 text-white border-none rounded-md cursor-pointer text-sm flex items-center
//                 ${!poPreviewContent || isBotTyping || isBotSpeaking ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-500'}`}
//               onClick={handleSendPo}
//               disabled={!poPreviewContent || isBotTyping || isBotSpeaking}
//             ><Send size={18} className="mr-1.5" /> Send PO</button>
//           </div>
//           <div className="flex-1 p-5 overflow-y-auto text-sm text-gray-700 flex items-center justify-center">
//             {poPreviewContent === "1" ? (<img src={GeneratedPO1} alt="PO Preview - Supplier Details" className="max-w-full max-h-full h-auto object-contain" />)
//               : poPreviewContent === "2" ? (<img src={GeneratedPO2} alt="PO Preview - Delivery Address" className="max-w-full max-h-full h-auto object-contain" />)
//                 : poPreviewContent === "3" ? (<img src={GeneratedPO3} alt="PO Preview - Final" className="max-w-full max-h-full h-auto object-contain" />)
//                   : (<div className="flex flex-col items-center justify-center text-gray-500 text-center">
//                     <DocumentIcon size={48} className="mb-2.5" /><div>Purchase Order Preview will appear here...</div></div>)}
//           </div>
//         </div>
//       </div>
//       <style>{`
